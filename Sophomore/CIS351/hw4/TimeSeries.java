...
public class TimeSeries {
  ...
  public int timesAbove(int v, int start, int end) {
    try{
      int result = 0;
      for(int i = start; i < end+1; i++){ //loop through the TimeSeries starting from start
        if(this.get(i) > v){
          result++; //add one to the counter if the value is above v.
        }
      }
      return result;
    }
    catch(IndexOutOfBoundsException e){
      System.out.println("Time interval was out of bounds! Returning 0...");
      return 0;
    }
  }
  ...
  public int oscillation(int start, int end) {
    try{
      int min = 9999999;
      int max = 0;
      for(int i = start; i < end + 1; i++){ //loop through the TimeSeries to determine the min and max values
        if(this.get(i) > max){
          max = this.get(i);
        }
        if(this.get(i) < min){
      ...
      int result = max - min; //subtract the min and max before returning the answer.
      return result;
    }
    catch(IndexOutOfBoundsException e){
      System.out.println("Time interval was out of bounds! Returning 0...");
  ...
  public TimeSeries runningAvg(int k) {
    ...
    try{
      ...
      System.out.println("Time interval was out of bounds! Returning empty TimeSeries...");
      TimeSeries error = new TimeSeries();
      return error;
    }
  }
}