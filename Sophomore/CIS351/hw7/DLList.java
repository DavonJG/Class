...
public class DLList<T> extends AbstractSequentialList<T> {
  ...
  public DLList<T> reverseCopy() {
    DLList<T> ans = new DLList<T>();
    for(int i = this.size() -1; i >=0; i--){ //add to the new list backwards to return a reversed copy.
      ans.add(this.get(i));
    }
    return ans;
  }
  ...
  public void reverse() {
    ...
    list = this.reverseCopy();
    for(int i=0; i < this.size(); i++){ //loop through the list and reverse it.
  ...
  public static DLList<Integer> merge(DLList<Integer> lst1, DLList<Integer> lst2) {
    ...
    if(lst1.n == 0 && lst2.n == 0){
      ...
      while((lst1.n != 0) || (lst2.n != 0)){
        ...
        else if (lst2.n == 0 && lst1.n != 0){
          ans.takeFirst(lst1);
        }
        else if (lst1.get(0) < lst2.get(0)){
          ans.takeFirst(lst1);
        }
        else if (lst2.get(0) < lst1.get(0)){
          ans.takeFirst(lst2);
        }
      }
      return ans;
  ...
  public static DLList<Integer> mergesort(DLList<Integer> lst1) {
    ...
    if(lst1.n == 0){
      ...
      list = lst1.splitAt(lst1.n/2);
      ans = merge((mergesort(lst1)), (mergesort(list)));
    }
    return ans;
  }
  //////////////////////////////////////////////////////////////////// 
