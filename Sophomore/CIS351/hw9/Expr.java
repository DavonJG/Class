...
public class Expr {
  ...
  public String toInfix() {
    ...
    switch (tag){
     case NUM: 
       ...
       return ans;
     case VAR:
       ans += this.name; //add the variable to ans
       return ans;
     default:
       ans += "("; //opens parentheses
       ans += left.toInfix() + " "; 
       ans += this.name + " "; 
       ans += right.toInfix() + " ";
       ans += ")";  //close
  ...
  public String toPostfix() {
    ...
    switch (tag){
      case NUM: 
        ans += this.num; //add the num to ans
        return ans;
      case VAR:
        ans += this.name; 
        return ans;
  ...
  public Expr simplify() {
    switch (tag){
      case NUM:
        return new Expr(this.num);
      case VAR:
        return new Expr(this.name);
      default:
        Expr x = this.left.simplify();
        Expr y = this.right.simplify();
        if (x.tag == Tag.NUM && y.tag == Tag.NUM){
          switch (name){
            case '+':  
              return new Expr(x.num + y.num);
            case '-':  
              return new Expr(x.num - y.num);
            case '*':  
              return new Expr(x.num * y.num);
