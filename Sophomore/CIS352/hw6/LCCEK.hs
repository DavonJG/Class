...
data ToDo = DoAOp1 AExp AOp | DoAOp2 AOp Int 
          ...
          | DoRep1 Command BExp | DoRep2 Command BExp
            deriving (Show)
...
-- commands: repeat
step (C (Repeat c be), s,ks)               = (C c,s,(DoRep1 c be):ks)
step (C Skip,          s,(DoRep1 c be):ks) = (B be,s,(DoRep2 c be):ks)
step (B (BConst True), s,(DoRep2 c be):ks) = (C Skip,s,ks)
step (B (BConst False),s,(DoRep2 c be):ks) = (C (Repeat c be),s,ks)
