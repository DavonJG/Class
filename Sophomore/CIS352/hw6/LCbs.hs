...
evalC (Repeat cmd be,s)         = evalC (cmd', s')
    where (BConst b, s')        = evalB (be, s)
          cmd'                  = if b then Skip else (Seq cmd (Repeat cmd be))
...
--   N.B. The test commands have no while-loops or repeat.
...
cmd9 = "repeat { x0 := val(x0)-1; x1 := val(x1)+3 } until (val(x0) <= 0) "