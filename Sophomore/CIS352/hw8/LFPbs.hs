...
freeVars (Var x)           = [x]
freeVars (Fun x e)         = nub (tail (union [x] (freeVars e)))
freeVars (Let x e1 e2)     = nub (tail (union [x] (freeVars e1)) ++ tail (union [x] (freeVars e2)))
...
subst (Var y)             x new
    = if y == x then new else (Var y)
subst (Fun y e)           x new
    = if x == y then (Fun y e) else (Fun z (subst (subst e y (Var z)) x new))
       where z = fresh (union (union (freeVars e) (freeVars new)) [x])
subst (Let y e0 e1)       x new
    = if y == x then (Let y (subst e0 x new) e1) else (Let z (subst e0 x new) (subst (subst e1 y (Var z)) x new))
       where z  = fresh (union (union (freeVars e0) (freeVars e1)) (freeVars new))
...
--{-
eval (App e1 e2,s)         = eval (AConst v,s'')
    where ((Fun x e1'),s') = eval (e1,s)
          (AConst v,s'')    = eval ((subst e1' x e2),s')   
---}
{-
-- call by value application
eval (App e1 e2,s)         = eval (v,s''')
    where ((Fun x e1'),s') = eval (e1,s)
          (v2,s'') = eval (e2,s')
          (v,s''')  = eval ((subst e1' x v2),s'')
-}
...
et11 = runEval "let a = 14 in (let c = (+ (fn b => a) b) in (let x = (+ 3 a) in (c b)))"

et12 = runEval "((fn x => (let x = (+ x 7) in (+ x y))) (+ x y))"