...
{-
eval env (Var x,s)     = (elookup x env, s)
eval env (Fun x e,s)   = (Fun x e, s)
eval env (App e1 e2,s) = (valToo, s3)
     where (Fun x e1', s1)               = eval env (e1,s)
           (val,s2)                      = eval env (e2,s1)
           (valToo,s3)                   = eval (extend env x val) (e1',s2)
-}
...
{-
eval env (Var x,s)     = (nameToo, s1)
     where (nameToo, s1)                 = eval env' (e, s)
           (Closure e env') = elookup x env
eval env (Fun x e,s)   = (Closure (Fun x e) env, s)
eval env (App e1 e2,s) = (appToo, s2)
     where (Closure (Fun x e1') env',s1) = eval env (e1,s)
           (appToo, s2)                  = eval (extend env' x (Closure e2 env)) (e1', s1)    
-}
...
eval env (Var x,s)     = (nameToo, s1)
     where (nameToo, s1)                 = eval env (elookup x env, s)
eval env (Fun x e,s)   = (Fun x e, s)
eval env (App e1 e2,s) = (appToo, s2)
     where (Fun x e1', s1)               = eval env (e1, s)
           (appToo, s2)                  = eval (extend env x e2) (e1', s1)
