-- LCcompiler.hs
-- Davon Grant
--   Version 5 (Tue Mar 1 13:06:41 EST 2016)
...
-- Homework 7
transB (Compare Leq ie1 ie2) = transA ie2 ++ transA ie1 ++ [Ilt] ++ [Nand]
-- Homework 7
transB (Compare Geq ie1 ie2) = transA ie1 ++ transA ie2 ++ [Ilt] ++ [Nand]
...
-- For Homework 7
transC (Repeat cmd be) 
    = let body = transC cmd
          thenpart = transB (Not be)
      in body ++ thenpart ++ [Jz (3+codeLen body)] ++ body
        ++ [Jmp (-(3+codeLen body + codeLen thenpart))]
...
c9 = "{ x0 := 3; repeat x0 := val(x0)-1 until val(x0)==0 }"
c10 = "{ x0:=4; x1:=1; repeat { x1 := val(x1)*val(x0); x0 := val(x0)-1 } "
     ++ "until val(x0)<1 }"
